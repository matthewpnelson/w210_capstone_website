...
def grab_similar_beers(index, neighbs = 5):
    new_data_point = np.append(buildDescVector(google_model, beers.loc[index, 'DescriptionTokenized'], vector_dim),[beers.loc[index, 'abv'],beers.loc[index, 'ibu'],beers.loc[index, 'rating'], beers.loc[index, 'style_enc']]).reshape([-1,304])
