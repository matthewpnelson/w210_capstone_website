...
app = dash.Dash(name='explorer_styles', sharing=True, server=server, url_base_pathname='/explorer_styles')
            ...
            [
                html.Div(
                    [
                        ...
                        html.P('Choose a Beer Style Here:'),
                        html.Div(
                            [

                                dcc.RadioItems(
                                    id='beer_style_selector',
                                    options=[
                                        # {'label': 'All ', 'value': 'all'},
...
def filter_brewery(rating_slider, ibu_slider, abv_slider, beer_style_dropdown):
...
# WORKING
# Radio -> multi
@app.callback(Output('beer_style_dropdown', 'value'),
              [Input('beer_style_selector', 'value')])
def display_status(selector):
    if selector == 'all':
        return beers['style'].unique()
    if selector == 'random':
        return [x for x in random.sample(set(beers['style'].unique()),1)]
    else:
        return []

# WORKING
# Radio -> multi
@app.callback(Output('brewery_dropdown', 'value'),
              [Input('brewery_selector', 'value'),
              Input('rating_slider', 'value'),
              Input('ibu_slider', 'value'),
              Input('abv_slider', 'value'),
              Input('beer_style_dropdown', 'value')])
def display_status(selector, rating_slider, ibu_slider, abv_slider, beer_style_dropdown):
    if selector == 'all':
        dff = beers[(beers['rating'] > rating_slider[0])
                            & (beers['rating'] < rating_slider[1])
                            & (beers['ibu'] > ibu_slider[0])
                            ...
                            & (beers['abv'] > abv_slider[0])
                            & (beers['abv'] < abv_slider[1])
                            & (beers['style'].isin(beer_style_dropdown))]
        return dff['brewery'].unique()
    if selector == 'random':
        dff = beers[(beers['rating'] > rating_slider[0])
                            & (beers['rating'] < rating_slider[1])
                            & (beers['ibu'] > ibu_slider[0])
                            ...
                            & (beers['abv'] < abv_slider[1])
                            & (beers['style'].isin(beer_style_dropdown))]
        output = [x for x in random.sample(set(dff['brewery'].unique()),20)]
        if output == []:
            return ['Odd 13 Brewing']
        else:
            return output
    else:
        return []
...
def make_main_figure(rating_slider, ibu_slider, abv_slider, beer_style_dropdown, brewery_dropdown, plot_colors_selector):
    ...
    if plot_colors_selector == 'Style':
        layout_individual['showlegend'] = True
